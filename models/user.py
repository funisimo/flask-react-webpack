...
class User(ndb.Model):
    ...
    def create(cls, email_address, password=None, admin=False, first_name=None, last_name=None):
        with client.context():
            ...
            if not user:  # if user does not yet exist, create one
                ...
                user = cls(email_address=email_address.lower(), password_hash=hashed, admin=admin,
    ...
    @classmethod
    def fetch_all(cls, limit=None, cursor=None):
        try:
            print('got to function')
            with client.context():
                users, next_cursor, more = cls.query().fetch_page(limit, start_cursor=cursor)
                print(users, next_cursor, more)
                try:
                    return users, next_cursor.urlsafe().decode(), more
                except AttributeError as e:  # if there's no next_cursor, an AttributeError will occur
                    return users, None, False
    ...
    def permanently_batch_delete(cls):
        ...
        with client.context():
            users_keys = cls.query(cls.deleted == True,
                                   cls.deleted_date < (datetime.datetime.now() - datetime.timedelta(days=30))).fetch(
                keys_only=True)
    ...
    def verify_email_address(cls, email):
        ...
        with client.context():
            user = cls.query(cls.email_address == email).get()
    ...
    def validate_password_login(cls, email_address, password, request=None):
        ...
        with client.context():
            ...
            if not bcrypt.checkpw(password.encode("utf-8"), user.password_hash.encode("utf-8")):
                return False, "The entered password is incorrect." + password.encode("utf-8") + " ----- " + user.password_hash.encode("utf-8")
